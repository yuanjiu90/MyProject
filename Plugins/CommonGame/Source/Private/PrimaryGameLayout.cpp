...
void UPrimaryGameLayout::RegisterLayer(FGameplayTag LayerTag, UCommonActivatableWidgetContainerBase* LayerWidget)
{
	if (!IsDesignTime())
	{
		...
		LayerWidget->OnDisplayedWidgetChanged().AddUObject(this, &UPrimaryGameLayout::OnDisplayedWidgetChanged);
...
void UPrimaryGameLayout::OnDisplayedWidgetChanged(UCommonActivatableWidget* DisplayedWidget)
{
	//UE_LOG(LogCommonGame, Warning, TEXT("UPrimaryGameLayout::OnDisplayedWidgetChanged [%s]"), DisplayedWidget? *DisplayedWidget->GetName(): TEXT("nullptr"));
	OnDisplayedWidgetChangedEvent.Broadcast(DisplayedWidget);
}
...
void UPrimaryGameLayout::RemoveAllWidgetFromLayer(const FGameplayTag LayerName)
{
	if (Layers.Contains(LayerName))
	{
		Layers[LayerName]->ClearWidgets();
	}
}
