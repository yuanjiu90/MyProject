...
bool UGameplayMessageSubsystem::HasInstance(const UObject* WorldContextObject)
{
	...
	{
		UE_LOG(LogGameplayMessageSubsystem, Warning, TEXT("HasInstance called with invalid WorldContextObject"));
		return false;
	}
	const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::Assert);
	const UGameplayMessageSubsystem* Router = World != nullptr ? UGameInstance::GetSubsystem<UGameplayMessageSubsystem>(World->GetGameInstance()) : nullptr;
...
DEFINE_FUNCTION(UGameplayMessageSubsystem::execK2_BroadcastMessage)
{
	...
	FSimpleParamMessage SimpleParam;
	...
	if (StructProp == nullptr)
	{
		...
		{
			SimpleParam = FSimpleParamMessage();
		}
		else
		{
			SimpleParam = FSimpleParamMessage(Stack.MostRecentProperty,MessagePtr);			
		}
	}
	...
	{
		P_THIS->BroadcastMessageInternal(Channel, FSimpleParamMessage::StaticStruct(), &SimpleParam);
	}
}

void UGameplayMessageSubsystem::BroadcastMessageWithNoPayload(FGameplayTag Channel)
{
	static const FSimpleParamMessage EmptyMessage = FSimpleParamMessage();
	BroadcastMessageInternal(Channel,FSimpleParamMessage::StaticStruct(),&EmptyMessage);
}
...
void UGameplayMessageSubsystem::UnregisterListenerInternal(FGameplayTag Channel, int32 HandleID)
{
	...
	{
		const int32 MatchIndex = pList->Listeners.IndexOfByPredicate([ID = HandleID](const FGameplayMessageListenerData& Other) { return Other.HandleID == ID; });
