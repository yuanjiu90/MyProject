...
UAsyncAction_ListenForGameplayMessage* UAsyncAction_ListenForGameplayMessage::ListenForSimpleGameplayMessages(
	UObject* WorldContextObject, FGameplayTag Channel, EGameplayMessageMatch MatchType)
{

	return ListenForGameplayMessages(WorldContextObject,Channel,FSimpleParamMessage::StaticStruct(), MatchType);
}

UAsyncAction_ListenForGameplayMessage* UAsyncAction_ListenForGameplayMessage::ListenForGameplayMessagesWithNoPayload(
	UObject* WorldContextObject, FGameplayTag Channel, EGameplayMessageMatch MatchType)
{
	return ListenForGameplayMessages(WorldContextObject,Channel,FSimpleParamMessage::StaticStruct(), MatchType);
}
...
{
	if (UObject* WorldContext = WorldContextPtr.Get())
	{
		if (UGameplayMessageSubsystem::HasInstance(WorldContext))
		{
			UGameplayMessageSubsystem& Router = UGameplayMessageSubsystem::Get(WorldContext);
...
void UAsyncAction_ListenForGameplayMessage::Unregister()
{
	SetReadyToDestroy();
}

FString UAsyncAction_ListenForGameplayMessage::GetDesc()
{
	FString Desc = Super::GetDesc();
	if (ChannelToRegister.IsValid())
	{
		Desc.Append("Tag: ");
		Desc.Append(ChannelToRegister.GetTagName().ToString());
	}
	if (IsValid(WorldContextPtr.Get()))
	{
		Desc.Append(";Context: ");
		Desc.Append(WorldContextPtr->GetName());
	}
	else
	{
		Desc.Append(";Error: Missing Call Unregister");
	}
	return Desc;
}
