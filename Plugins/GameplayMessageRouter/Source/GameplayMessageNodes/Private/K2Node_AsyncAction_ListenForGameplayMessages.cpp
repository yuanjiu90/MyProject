...
namespace UK2Node_AsyncAction_ListenForGameplayMessagesHelper
{
	...
	static FName WithNoPayloadFunctionName = "ListenForGameplayMessagesWithNoPayload";
	static FName SimpleParamFunctionName = "ListenForSimpleGameplayMessages";
};
	...
	struct GetMenuActions_Utils
	{
		static void SetNodeFunc(UEdGraphNode* NewNode, bool /*bIsTemplateNode*/, TWeakObjectPtr<UFunction> FunctionPtr)
		{
			...
			if (FunctionPtr.IsValid())
			{
				...
				AsyncTaskNode->SetNodeTypeWithProxyFunctionName();
			}
		}
	};
...
void UK2Node_AsyncAction_ListenForGameplayMessages::AllocateDefaultPins()
{
	...
	InitializePinsByFunctionName();

}

void UK2Node_AsyncAction_ListenForGameplayMessages::SetNodeTypeWithProxyFunctionName()
{
	if (ProxyFactoryFunctionName == UK2Node_AsyncAction_ListenForGameplayMessagesHelper::WithNoPayloadFunctionName)
	{
		bWithoutPayload = true;
	}
	if (ProxyFactoryFunctionName == UK2Node_AsyncAction_ListenForGameplayMessagesHelper::SimpleParamFunctionName)
	{
		bUseSimpleParam = true;
	}
}
...
void UK2Node_AsyncAction_ListenForGameplayMessages::RefreshOutputPayloadType() const
{
	UEdGraphPin* PayloadPin = GetPayloadPin();
	const UEdGraphPin* PayloadTypePin = GetPayloadTypePin();
	if (PayloadTypePin == nullptr)
	{
		return;
	}

	if (PayloadTypePin->DefaultObject != PayloadPin->PinType.PinSubCategoryObject.Get())
	{
...
void UK2Node_AsyncAction_ListenForGameplayMessages::InitializePinsByFunctionName()
{
	SetNodeTypeWithProxyFunctionName();
	const UEdGraphPin* PayloadTypePin = GetPayloadTypePin();
	UEdGraphPin* PayloadPin = GetPayloadPin();
	...
	if (PayloadTypePin == nullptr)
	{
		...
		if (bUseSimpleParam)
		{
			...
			PayloadPin->PinType.PinSubCategoryObject = FSimpleParamMessage::StaticStruct();
			PayloadPin->PinType.PinCategory = UEdGraphSchema_K2::PC_Struct;
			return;
		}
		UE_LOG(LogTemp, Warning , TEXT("节点过时或有问题，请检查"))
	}
}

UEdGraphPin* UK2Node_AsyncAction_ListenForGameplayMessages::GetPayloadPin() const
{
	UEdGraphPin* Pin = FindPin(UK2Node_AsyncAction_ListenForGameplayMessagesHelper::PayloadPinName);
	if (Pin)
	{
...
{
	UEdGraphPin* Pin = FindPin(UK2Node_AsyncAction_ListenForGameplayMessagesHelper::PayloadTypePinName);
	if (Pin)
	{
